...
void sr_add_to_advertisements(uint32_t subnet, uint32_t mask);
		...
		if (head)
		{
			...
			while (iflist)
			{
				while (temp)
				{
					...
					sr_add_to_advertisements(subnet, mask);
				...
				lsuhdr->head = lsu_head;
				uint8_t *pkt = (uint8_t *)malloc(sizeof(struct sr_ethernet_hdr) + sizeof(struct ip) + sizeof(struct ospfv2_hdr)
				...
				sr_send_packet(sr, pkt, sizeof(struct sr_ethernet_hdr) + sizeof(struct ip) + sizeof(struct ospfv2_hdr)
				               + sizeof(struct ospfv2_lsu_hdr), intf);
